...
export function AnimalLibraryScreen({ onClose }: Props) {
  ...
  const sorted = useMemo(() => [...animalsData].sort((a, b) => a.name.localeCompare(b.name, 'tr')), []);
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Anadolu'dan dünyaya · {animalsData.length} hayvan rehberi</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {groups.length === 0 && <Text style={styles.empty}>Arama sonucu yok.</Text>}
        {groups.map(([letter, items]) => (
          <View key={letter} style={styles.group}>
            ...
            {items.map(animal => (
              <AnimalRow key={animal.id} animal={animal} onPress={() => setSelected(animal)} />
            ))}
          </View>
        ))}
  ...
  return (
    <TouchableOpacity style={styles.row} onPress={onPress} activeOpacity={0.7}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.rowMeta}>{animal.element} · {animal.symbolism.slice(0, 2).join(' · ')}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  searchBox: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, marginHorizontal: Spacing.lg, paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm, backgroundColor: Colors.backgroundCard, borderWidth: 1, borderColor: Colors.cardBorder, borderRadius: BorderRadius.round, marginBottom: Spacing.md },
  ...
  groupLetter: { fontSize: Typography.size.sm, fontWeight: Typography.weight.semibold, color: Colors.tealLight, letterSpacing: 2 },
  groupLine: { flex: 1, height: 1, backgroundColor: Colors.divider },
  row: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md, paddingVertical: Spacing.sm, paddingHorizontal: Spacing.sm, borderBottomWidth: 1, borderBottomColor: Colors.divider },
  rowImgWrap: { width: 44, height: 44, borderRadius: 22, borderWidth: 1, borderColor: Colors.teal + '40', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', backgroundColor: Colors.backgroundCard },
